...
public interface FriendshipRepository extends JpaRepository<Friendship, Long> {
    ...
    @Query(value = """
        SELECT u.* FROM mxh.users u
        WHERE u.id IN (
            SELECT CASE 
                WHEN f.requester_id = :myId THEN f.addressee_id 
                ELSE f.requester_id 
            END
            FROM mxh.friendships f
    ...
    @Query(value = """
    SELECT u.* FROM mxh.users u
    JOIN mxh.friendships f ON (u.id = f.requester_id OR u.id = f.addressee_id)
